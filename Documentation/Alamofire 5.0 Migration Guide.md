...
- `MultipartFormData`’s API has changed and the top level `upload` methods to create and upload `MultipartFormData` have been updated to match other request APIs, so it’s no longer necessary to deal with the `Result` of the multipart encoding.
